# main.py
import os, io, uuid, threading, time, datetime as dt
from urllib.parse import urljoin
from flask import Flask, request, jsonify, send_file, render_template
import pandas as pd

from agents import (
    EndpointCollector, Checker, Fixer, AdvisorAI,
    write_with_images_fit
)

app = Flask(__name__, template_folder="templates", static_folder=None)
JOBS = {}      # job_id -> dict
REPORTS = {}   # job_id -> bytes
    ...
    try:
        ...
        for i, ep in enumerate(eps, 1):
            ...
            results.append({
                ...
                "HTTP": pr["http_status"],
                "tester": "Mongoo",
                "tested_at": dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            })
...
def run_test():
    ...
    JOBS[job_id] = {"ok": True, "status": "queued", "progress": 0, "total": 0, "summary": {"passed":0,"failed":0}, "errors":{}}

    t = threading.Thread(target=run_job, args=(
        job_id, base_url, project_root, endpoints_text, excel_path, login_url, login_id, login_pw
    ), daemon=True)
    t.start()
...
if __name__ == "__main__":
    port = int(os.environ.get("PORT", "5051"))  # avoid 5000
    # Bind to loopback and disable reloader to prevent double-bind on Windows
